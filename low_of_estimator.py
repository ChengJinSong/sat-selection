...
TF_MODEL_PATH = PATH_ROOT + 'tmp_model/SAT12-INDU-3/'
# LOGFILE = PATH_ROOT + 'tmp_model/log/SAT12-INDU.log'

TOTAL_STEPS = 1600
THRESHOLD = [0.5]
BATCH_SIZE_OF_TRAIN = 64
BATCH_SIZE_OF_EVAL = 139
# LEARNING_RATE = 0.08
# MOMENTUM = 0.96
NUM_LABEL = 31
NUM_BATCH_OF_EPOCHS = 16
NUM_EPOCH = int(TOTAL_STEPS / NUM_BATCH_OF_EPOCHS)
IMG_SIZE = [128, 128]
...
def cnn_model(features, labels, mode, params):
    ...
    # lr_decay = tf.to_float(tf.floor_div(gs, 10))*lr_step
    # mom_decay = tf.to_float(tf.floor_div(gs, 10))*mom_step
    log_lr = tf.logical_and(tf.less(gs, 1000), tf.equal(tf.mod(gs, 10), 0))
    log_mom = tf.logical_and(tf.less(gs, 1000), tf.equal(tf.mod(gs, 10), 0))

    lr = tf.cond(log_lr, lambda: tf.assign_add(lr, lr_step), lambda: lr)
    mom = tf.cond(log_mom, lambda: tf.assign_add(mom, mom_step), lambda: mom)
...
def main():
    # get data
    input_fn, eval_data = input_data(True)
    ...
    tensors_to_log = {"learning_rate": "learning_rate", "momentum": "momentum"}
    ...
    #     shuffle=True)
    sat_clf.train(input_fn=input_fn, hooks=[logging_hook])
    ...
    # with open('/home/song/Data/tmp_model/pre_all.pickle', 'wb') as f:
    #     pk.dump(list(test_res), f)
    # print(list(test_res))
